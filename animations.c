...
void animation_credits(int state) {
  switch (state) {
  case GL4DH_INIT:
...
void animation_systeme_solaire(int state) {
  switch (state) {
  case GL4DH_INIT:
    systeme_solaire_init();
    return;
  case GL4DH_FREE:
    systeme_solaire_sortie();
    return;
  case GL4DH_UPDATE_WITH_AUDIO:
    return;
  default: /* GL4DH_DRAW */
    systeme_solaire_draw();
    return;
  }
}

void animation_saturne(int state) {
  switch (state) {
  case GL4DH_INIT:
    saturne_init();
    return;
...
void animation_terre(int state) {
  switch (state) {
  case GL4DH_INIT:
    terre_init();
    return;
