...
void main(void) {
  ...
  for (int i = 0; i < 19; ++i){
    ...
    n += A * noise(vec2(tcoord[1], tcoord[0]) * 3.5f * mf);
    A = A/2.0;
    mf = mf * 2.0;
  }

  vec4 mycolor = vec4(n);
  fragColor = (vec4(0.9, 0.9, 0.9, 0.0)+ mycolor)*vec4(0.6, 0.6, 0.6, 0.0)*ild;
  /* if (mycolor[0] > 0.001){ */
  /*   // le *vec4(-1) permet d'inversé le côté clair */
  /*   fragColor = vec4(0.2, 0.4, 0.0, 0.0) + mycolor;//\*vec4(-1); */
  /* } else { */
  /*   // le *vec4(-1) permet d'inversé le côté sombre */
  /*   fragColor = vec4(0.0, 0.0, 1.0, 1.0)+ mycolor;//\*vec4(-1); */
    
  /* } */

    /* if (mycolor[0] < 0.4  && mycolor[1] < 0.4 && mycolor[2] < 0.4){ */
    
    /*   fragColor = vec4(0.1, 0.1, 0.5, 0.0)* mycolor; */
    /* } else { */
    /*   fragColor = vec4(0.0, 0.0, 0.0, 0.0) * mycolor; */
...
/*   vec4 mycolor = vec4(n); */
/*   if (mycolor[0] > 0.8  && mycolor[1] > 0.8 && mycolor[2] > 0.8){ */
/*     fragColor = vec4(1.0, 1.0, 1.0, 1.0);// + mycolor; */
/*   } else { */
/*     fragColor = vec4(0.0, 0.0, 0.0, 0.0); //mycolor; */
